    ...
    <div className="bg-[#fafbfc]">
      ...
      <section className="py-10 md:py-4 bg-[#fafbfc]">
        <div className="max-w-6xl mx-auto px-4">
          <div className="text-center mb-8" data-aos="fade-up"></div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            {landingVersions.map((version, index) => (
              <div key={version.id} className="relative group h-full" data-aos="fade-up" data-aos-delay={index * 100}>
                <div className="relative bg-white rounded-3xl p-6 shadow-sm border border-gray-200 flex flex-col h-full hover:bg-gray-50 hover:border-gray-300 hover:shadow-md transition-all duration-500 ease-out">
                  {/* Header - icono gris */}
                  <div className="w-14 h-14 rounded-2xl bg-gray-100 flex items-center justify-center mb-4 group-hover:bg-gray-200 transition-colors duration-500">
                    <version.icon className="w-7 h-7 text-gray-600" />
                  </div>
                  ...
                  <p className="text-2xl font-bold text-[#0d233f] mb-3">{version.price}</p>

                  <p className="text-[#475569] text-sm mb-5">{version.description}</p>

                  {/* Features - checks grises */}
                  <div className="space-y-2 mb-5 flex-grow">
                    {version.features.map((feature, idx) => (
                      <div key={idx} className="flex items-start gap-2">
                        <div className="w-5 h-5 rounded-full bg-gray-200 flex items-center justify-center flex-shrink-0 mt-0.5 group-hover:bg-gray-300 transition-colors duration-500">
                    ...
                    <div className="mb-5">
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {version.tabExamples.map((tab, idx) => (
                          <span
                            key={idx}
                            className="bg-gray-100 text-[#0d233f] text-xs px-2 py-1 rounded-full border border-gray-200"
                          >
                            {tab}
                          </span>
                        ))}
                  ...
                  <div className="flex flex-col gap-2">
                    <Link
                      to={version.route}
                      className="inline-flex items-center justify-center gap-2 bg-white border-2 border-gray-300 text-[#0d233f] px-5 py-2.5 rounded-xl font-semibold hover:bg-gray-50 hover:border-gray-400 transition-all duration-300 w-full"
                    >
                      <span className="font-semibold text-[#0d233f]">Ver más</span>
                      <ArrowRight className="w-4 h-4 text-[#0d233f]" />
                    </Link>
                    <a
      ...
      {/* CTA Section - botón sin gradiente */}
      <section className="py-10 md:py-14 bg-[#fafbfc]">
        <div className="max-w-4xl mx-auto px-4 text-center" data-aos="fade-up">
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-3 bg-[#0d233f] text-white px-8 py-4 rounded-xl font-semibold hover:bg-[#1a3a5c] hover:shadow-md transition-all duration-300"
          >
